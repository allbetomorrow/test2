...
const FieldWrapper = ({ children, dispatch, isActive }: FieldWrapperProps) => {
  return (
    <div className="flex items-center mt-3 max-w-lg text-center text-light-sec dark:text-dark-sec ">
      {children}
      <button className="ml-auto" type="button" onClick={dispatch}>
        {isActive
          ? <svg className="w-7 h-7 dark:fill-dark-sec" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
    ...
    >
      {({ isSubmitting, values }) => (
        ...
        <Form className="flex flex-col mb-3">
          ...
          <FieldWrapper dispatch={() => dispatch({ type: "contentActive" })} isActive={state.contentActive}>
            {state.contentActive
              ? <Field name="content" type="text" as="textarea"
                className="text-left w-full dark:bg-dark-main h-28 px-3 py-2 mr-1 border-dark-th
                resize-none border rounded  outline-none "
              />
              : <p
                className="text-left w-full px-3 py-2 mr-1 h-28 overflow-y-auto border rounded border-dark-th"
              >
                {values.content}
              </p>
            }
